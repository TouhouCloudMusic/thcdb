...
use super::error::{
    AuthnBackendError, InvalidEmail, ResendVerificationEmailError,
    SessionBackendError, SessionError, SignInError, SignUpError,
    VerifyEmailError,
};
use super::{Email, repo};
use crate::domain::auth::{
    AuthCredential, AuthnError, HashedPassword, ResendVerificationEmailRequest,
    ResendVerificationEmailResponse, SIGNUP_EXPIRES_HOURS, SignUpRequest,
...
use crate::domain::user::{self, NewUser, User};
...
#[derive(Debug)]
enum SendVerificationEmailError {
    Infra(Error),
    Unavailable,
...
fn build_verification_email_message(
    from: Mailbox,
    to: Mailbox,
    code: &str,
...
fn is_unverified_signup_expired(user: &User) -> bool {
    if user.email_verified {
        return false;
    }
